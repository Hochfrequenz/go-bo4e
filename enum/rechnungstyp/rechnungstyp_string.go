// Code generated by "stringer --type Rechnungstyp"; DO NOT EDIT.

package rechnungstyp

import "strconv"

func _() {
	// An "invalid array index" compiler error signifies that the constant values have changed.
	// Re-run the stringer command to generate them again.
	var x [1]struct{}
	_ = x[ABSCHLAGSRECHNUNG-1]
	_ = x[TURNUSRECHNUNG-2]
	_ = x[MONATSRECHNUNG-3]
	_ = x[WIMRECHNUNG-4]
	_ = x[ZWISCHENRECHNUNG-5]
	_ = x[INTEGRIERTE_13TE_RECHNUNG-6]
	_ = x[ZUSAETZLICHE_13TE_RECHNUNG-7]
	_ = x[MEHRMINDERMENGENRECHNUNG-8]
	_ = x[ABSCHLUSSRECHNUNG-9]
	_ = x[MSBRECHNUNG-10]
	_ = x[KAPAZITAETSRECHNUNG-11]
	_ = x[SONDERRECHNUNG-12]
	_ = x[SPERRUNG_INBETRIEBNAHME-13]
	_ = x[VERZUGSKOSTEN-14]
	_ = x[BLINDARBEIT-15]
	_ = x[ABRECHNUNG_VON_KONFIGURATIONEN_UNIVERSALBESTELLPROZESS-16]
	_ = x[ABRECHNUNG_TECHNIK-17]
}

const _Rechnungstyp_name = "ABSCHLAGSRECHNUNGTURNUSRECHNUNGMONATSRECHNUNGWIMRECHNUNGZWISCHENRECHNUNGINTEGRIERTE_13TE_RECHNUNGZUSAETZLICHE_13TE_RECHNUNGMEHRMINDERMENGENRECHNUNGABSCHLUSSRECHNUNGMSBRECHNUNGKAPAZITAETSRECHNUNGSONDERRECHNUNGSPERRUNG_INBETRIEBNAHMEVERZUGSKOSTENBLINDARBEITABRECHNUNG_VON_KONFIGURATIONEN_UNIVERSALBESTELLPROZESSABRECHNUNG_TECHNIK"

var _Rechnungstyp_index = [...]uint16{0, 17, 31, 45, 56, 72, 97, 123, 147, 164, 175, 194, 208, 231, 244, 255, 309, 327}

func (i Rechnungstyp) String() string {
	i -= 1
	if i < 0 || i >= Rechnungstyp(len(_Rechnungstyp_index)-1) {
		return "Rechnungstyp(" + strconv.FormatInt(int64(i+1), 10) + ")"
	}
	return _Rechnungstyp_name[_Rechnungstyp_index[i]:_Rechnungstyp_index[i+1]]
}
