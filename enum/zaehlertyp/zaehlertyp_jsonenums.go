...
func init() {
	...
	if _, ok := interface{}(v).(fmt.Stringer); ok {
		_ZaehlertypNameToValue = map[string]Zaehlertyp{
			...
			interface{}(MESSDATENREGISTRIERGERAET).(fmt.Stringer).String():      MESSDATENREGISTRIERGERAET,
			interface{}(ELEKTRONISCHERHAUSHALTSZAEHLER).(fmt.Stringer).String(): ELEKTRONISCHERHAUSHALTSZAEHLER,
			interface{}(SONDERAUSSTATTUNG).(fmt.Stringer).String():              SONDERAUSSTATTUNG,
		}
	}
}
